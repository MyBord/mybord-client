module.exports = {
  globals: {
    'ts-jest': {
      diagnostics: {
        ignoreCodes: [151001],
      },
    },
  },
  moduleFileExtensions: ['js', 'jsx', 'ts', 'tsx'],
  moduleDirectories: ['node_modules'],
  moduleNameMapper: {
    '\\.(jpg|jpeg|png|gif|eot|otf|webp|svg|ttf|woff|woff2|mp4|webm|wav|mp3|m4a|aac|oga)$':
      '<rootDir>/__mocks__/fileMock.js',
    '\\.(css|scss|less)$': 'identity-obj-proxy',
    '^alert(.*)$': '<rootDir>/src/spa/components/atoms/alert$1',
    '^animations(.*)$': '<rootDir>/src/spa/components/neutrons/animations$1',
    '^api(.*)$': '<rootDir>/src/api$1',
    '^app(.*)$': '<rootDir>/src/spa/app$1',
    '^atoms(.*)$': '<rootDir>/src/spa/components/atoms$1',
    '^backgrounds(.*)$': '<rootDir>/src/spa/components/neutrons/backgrounds$1',
    '^buttons(.*)$': '<rootDir>/src/spa/components/atoms/buttons$1',
    '^context(.*)$': '<rootDir>/src/context$1',
    '^copy(.*)$': '<rootDir>/src/spa/components/molecules/copy$1',
    '^fallbacks(.*)$': '<rootDir>/src/spa/components/atoms/fallbacks$1',
    '^footer(.*)$': '<rootDir>/src/spa/components/organisms/footer$1',
    '^form(.*)$': '<rootDir>/src/spa/components/atoms/form$1',
    '^formItem(.*)$': '<rootDir>/src/spa/components/atoms/formItem$1',
    '^forms(.*)$': '<rootDir>/src/spa/components/molecules/forms$1',
    '^graphics(.*)$': '<rootDir>/src/spa/components/neutrons/graphics$1',
    '^header(.*)$': '<rootDir>/src/spa/app/layout/header$1',
    '^hooks(.*)$': '<rootDir>/src/hooks$1',
    '^icon(.*)$': '<rootDir>/src/spa/components/neutrons/icon$1',
    '^images(.*)$': '<rootDir>/src/spa/components/neutrons/images$1',
    '^inputs(.*)$': '<rootDir>/src/spa/components/atoms/inputs$1',
    '^landing(.*)$': '<rootDir>/src/spa/landing$1',
    '^layout(.*)$': '<rootDir>/src/spa/app/layout$1',
    '^lottiePlayer(.*)$': '<rootDir>/src/spa/components/neutrons/lottiePlayer$1',
    '^lotties(.*)$': '<rootDir>/src/spa/components/neutrons/lotties$1',
    '^menus(.*)$': '<rootDir>/src/spa/components/molecules/menus$1',
    '^mockData(.*)$': '<rootDir>/src/mockData$1',
    '^modal(.*)$': '<rootDir>/src/spa/components/atoms/modals/modal$1',
    '^modals(.*)$': '<rootDir>/src/spa/components/molecules/modals/modals$1',
    '^molecules(.*)$': '<rootDir>/src/spa/components/molecules$1',
    '^navigation(.*)$': '<rootDir>/src/spa/app/layout/navigation$1',
    '^neutrons(.*)$': '<rootDir>/src/spa/components/neutrons$1',
    '^organisms(.*)$': '<rootDir>/src/spa/components/organisms$1',
    '^pages(.*)$': '<rootDir>/src/spa/app/pages$1',
    '^popOver(.*)$': '<rootDir>/src/spa/components/atoms/modals/popOver$1',
    '^portal(.*)$': '<rootDir>/src/spa/components/neutrons/portal$1',
    '^routing(.*)$': '<rootDir>/src/spa/app/routing$1',
    '^schema(.*)$': '<rootDir>/src/server/schema$1',
    '^server(.*)$': '<rootDir>/src/server$1',
    '^storybook(.*)$': '<rootDir>/src/thirdParty/storybook$1',
    '^styles(.*)$': '<rootDir>/src/styles$1',
    '^templates(.*)$': '<rootDir>/src/spa/components/templates$1',
    '^typography(.*)$': '<rootDir>/src/spa/components/atoms/typography$1',
    '^types(.*)$': '<rootDir>/src/types$1',
    '^userCard(.*)$': '<rootDir>/src/spa/components/organisms/userCard$1',
    '^utils(.*)$': '<rootDir>/src/utils$1',
  },
  setupFilesAfterEnv: ['<rootDir>/setupTests.js'],
  preset: 'ts-jest',
  transform: {
    '^.+\\.jsx?$': 'babel-jest',
    '^.+\\.tsx?$': 'ts-jest',
  },
};
